...
if (isset($_POST['app_id'])) {
	...
	if ($app_id && $secret) {
		$sql = "UPDATE facebook_connect SET app_id = ?, secret = ?, facebook_page = ? WHERE id = 1";
		$q = $pdo->prepare($sql);
		$q->execute(array($_POST['app_id'], $_POST['secret'], $_POST['facebook_page']));
		Database::disconnect();
	}
}
?>
...
if (($app_id != '') && ($secret != '')) {
	...
	if (@$_SESSION['facebook_access_token'] != '') {
		...
		// Update database
		$sql = "UPDATE facebook_connect SET access_token = ? WHERE id = 1";
		$q = $pdo->prepare($sql);
	...
	if ($access_token != '') {
		...
		if ($user->getProperty('id') != '') {
			...
			if (@$_SESSION['facebook_access_token'] != '') {
				// Update database
				$sql = "UPDATE facebook_connect SET user_id = ? WHERE id = 1";
				$q = $pdo->prepare($sql);
								...
								<div class="panel-body">
									<form class="form-horizontal" action="index.php" method="post">
										<div class="form-group">
											...
											<div class="col-sm-8">
												<input type="text" name="app_id" class="form-control" value="<?php echo $app_id; ?>" required>
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-8">
												<input type="text" name="secret" class="form-control" value="<?php echo $secret; ?>" required>
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-8">
												...
												<div><a href="<?php echo $loginUrl; ?>" onclick="return onClick(href)"><img src="<?php echo $relative_url; ?>assets/images/login_btn.png"></a></div>
												<div>
													<?php if ($user_id) { ?>
														<img style="margin:10px 0" src="https://graph.facebook.com/<?php echo $user_id; ?>/picture" alt="">
													<?php } ?>
												</div>
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-8">
												<input type="text" name="access_token" class="form-control" value="<?php echo $access_token; ?>" disabled>
											</div>
										</div>
													...
													if (isset($pagesToken['data'])) {
														if ($pagesToken['data']) { 
															foreach ($pagesToken['data'] as $option_page) { ?>
																<option value="<?php echo $option_page['id']; ?>" <?php echo ($option_page['id'] == $facebook_page) ? 'selected="selected"' : ''; ?>><?php echo $option_page['name']; ?> - [<?php echo $option_page['category']; ?>]</option>
															<?php
															}
														}
													}
